...
const mapMatchObj = (rootObj: any, path: string) => {
    ...
    if (rootObj.tagName) {
        ...
        GlobalObject.moveProperty(path, 'tagName', `${path}/match`, 'tagName');
    }

    if (rootObj.attributeName) {
        match.attributeName = rootObj.attributeName;
        delete rootObj.attributeName;
        GlobalObject.moveProperty(path, 'attributeName', `${path}/match`, 'attributeName');
    }

    if (rootObj.attributeValue) {
        match.attributeValue = rootObj.attributeValue;
        delete rootObj.attributeValue;
        GlobalObject.moveProperty(path, 'attributeValue', `${path}/match`, 'attributeValue');
    }
...
const htmlRuleMap = {
    ...
    'ltm html-rule tag-append-html': {
        ...
        customHandling: (rootObj: any, loc: any) => {
            ...
            rootObj.match = mapMatchObj(rootObj, globalPath);

            // Convert 'action' object
            if (rootObj.text) {
                rootObj.content = rootObj.text;
                delete rootObj.text;
                GlobalObject.moveProperty(globalPath, 'text', globalPath, 'content');
            }
    ...
    'ltm html-rule tag-remove-attribute': {
        ...
        customHandling: (rootObj: any, loc: any, file: any) => {
            ...
            GlobalObject.addProperty(globalPath, 'attributeName', loc.original, { attributeName: null });
            newObj[loc.profile] = rootObj;
            return newObj;
        }
    }
