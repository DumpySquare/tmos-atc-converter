    ...
    'gtm server': {
        ...
        customHandling: (rootObj, loc) => {
            ...
            if (rootObj.virtualServers) {
                rootObj.virtualServers = Object.keys(rootObj.virtualServers).map((name) => {
                    const addressPort = ipUtils.splitAddress(rootObj.virtualServers[name].destination);
                    addressPort.port = parseInt(addressPort.port, 10) || '';
                    if (name.match(/^\d+$/g)) return addressPort;
                    return Object.assign(addressPort, { name });
                });
            }
