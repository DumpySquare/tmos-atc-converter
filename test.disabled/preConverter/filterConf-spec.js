...
const filterConf = require('../../src/utils/filterConf');
...
describe('Test filterConf function (filterConf.js)', () => {
    ...
    it('should keep unsupported objects from config ', async () => {
        const ex0 = {
            'ltm test /AS3/test/test_obj': { 'app-service': 'none' },
            'ltm test2 /AS3/test2/test_obj2': { 'app-service': 'none' }
        };
        ...
        // apply whitelist for AS3 and ACC support
        const as3Json = filterConf(data, as3PropertiesExt, { excluded: true });
        assert.deepStrictEqual(as3Json, ex0);
    });
});
