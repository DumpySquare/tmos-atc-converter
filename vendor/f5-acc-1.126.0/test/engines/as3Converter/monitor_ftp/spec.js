...
describe('Monitor FTP: ltm monitor ftp', () => {
    it('monitor_ftp', async () => {
        const json = await as3ConvHelper.convertConfigFiles(`${__dirname}/monitor_ftp.conf`);
        ...
        return as3ConvHelper.compareAndValidate(
            json,
            await miscUtils.loadJSON(`${__dirname}/monitor_ftp.json`)
        );
    });
});
