...
|
| You can use the /info endpoint to retrieve information on the version and release of BIG-IP ACC, as well as the current version of the BIG-IP ACC schema (and minimum required schema version).
| Call the endpoint with ``curl localhost:8080/info``
|
For example:
...
* **--disable-analytics** option will disable usage reporting. All data is anonymized, and this option is enabled by default.  See :ref:`analytics` for more information.
...
Analytics are enabled by default. All data is collected in accordance with the `F5 Privacy Policy <https://www.f5.com/company/policies/privacy-notice>`_.
BIG-IP ACC collects analytics and statistics sending them to the TS TEEM (Telemetry, Entitlement, Enforcement, & Mediation) server. Telemetry support can be disabled with parameter **--disable-analytics**.
...
ACC has an option to enable process logging.

As an example, using ``curl localhost:8080/converter -X POST --form "conf=@../IN/test.conf" --form "verbose=true" --form "next=true" --form "jsonLogs=true" |jq`` results in the following logs:



::

    2023-10-19 12:09:36 INFO -------------------------------------
    2023-10-19 12:09:36 INFO ACC version:                   1.34.1
    2023-10-19 12:09:36 INFO AS3 core schema version:       3.47.0
    2023-10-19 12:09:36 INFO Shared schema version:         3.47.0
    2023-10-19 12:09:36 INFO Shared schema package version: 0.14.13
    2023-10-19 12:09:36 INFO -------------------------------------
    2023-10-19 12:09:36 WARN Ignoring "sys folder" (empty definitions array in f5-as3/properties)
    2023-10-19 12:09:38 INFO [
        {
            "path": "/Common/Shared/vs_http/disabled",
            "reason": "JSON Classic Schema Validator",
            "action": "REMOVE"
        },
        {
            "path": "/Common/Shared/vs_http_perf/disabled",
            "reason": "JSON Classic Schema Validator",
            "action": "REMOVE"
        },
        {
            "path": "/Common/Shared/vs_https_passthrough/disabled",
            "reason": "JSON Classic Schema Validator",
            "action": "REMOVE"
        }
    ]
    2023-10-19 12:09:38 INFO 175 BIG-IP objects detected total
    2023-10-19 12:09:38 INFO 36 BIG-IP objects recognized by AS3
    2023-10-19 12:09:38 INFO 31 BIG-IP objects supported by ACC
    2023-10-19 12:09:38 INFO 6 AS3 stanzas generated
