...
module.exports = (data, result, config) => {
    ...
    if (extraFields.engine === 'AS3') {
        if (config.next) {
            ...
            extraFields.as3NextUndefinedNotConverted = result.metadata.as3NextUndefinedNotConverted.length;
            const undefKeyCount = getKeyCount(result.metadata.as3NextUndefinedNotConverted);
            extraFields.as3NextUndefinedNotConvertedKeyCount = undefKeyCount;
