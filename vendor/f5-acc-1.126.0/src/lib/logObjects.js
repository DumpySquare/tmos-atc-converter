...
module.exports = (result, config) => {
    ...
    if (config.declarativeOnboarding) {
        ...
        // Log not recognized objects
        if (config.as3NotRecognized) {
            log.info('------- AS3-Not-Recognized objects ------');
            Object.keys(as3NotRecognized).forEach((x) => log.info(x));
            log.info('--- end of AS3-Not-Recognized objects ---');
        }
        ...
        if (config.next) {
            ...
            if (config.nextNotConverted) {
                ...
                as3NextUndefinedNotConverted.forEach((x) => log.info(x));
