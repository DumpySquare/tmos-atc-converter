    ...
    try {
        ...
        Object.keys(files).forEach((key) => {
            ...
            if (groupResult.error) {
                log.error(groupResult.error.message);
                throw new Error(groupResult.error.message);
            }

            const groupArr = groupResult.group.map((obj) => orchestrate(obj));
        ...
        // Check if the error is from groupObjects
        const isGroupObjectsiRuleError = e instanceof Error && e.message.startsWith('iRule parsing error');
        if (isGroupObjectsiRuleError) {
            // If it's from groupObjects iRule parsing, re-throw the error without the custom message
            throw e;
        } else {
